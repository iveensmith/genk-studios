...
import detailRoleweave from './assets/detail-roleweave.jpg'
import detailUniquePredict from './assets/detail-uniquepredict.jpg'
import detailVoiceIQ from './assets/detail-voiceiq.jpg'
import detailWorkflowAuth from './assets/detail-workflowauth.jpg'
...
const caseStudies = [
  {
    ...
    problem:
      'Job seekers send the same CV to every posting and hope it lands. Roleweave had to make tailoring a CV to a specific role take about a minute instead of an evening, without inventing anything the applicant can’t back up.',
    approach: [
      'A single master profile as the source of truth. Employers, titles, dates and metrics are pinned in code, so the model can reorder and reword but never fabricate.',
      'Two scores on every version: a Match score for fit against the posting, a Parse score for whether an ATS will read the file cleanly.',
      'One job in, a full application out: tailored CV, cover letter, recruiter note, company dossier and interview brief, all drawn from the same evidence.',
      'Six single-column, ATS-safe layouts, plus free in-browser tools that upload and store nothing.',
    ],
    outcome: [
      'Shipped end to end: the tailoring engine, Match and Parse scoring, six export layouts and subscription billing.',
      'A Job Radar digest that scores new postings against the master profile so the time goes into applying, not searching.',
    ],
  },
  {
    slug: 'uniquepredict',
    ...
    role: 'Model, product and build',
    url: 'https://unique-preditz.vercel.app/',
    image: caseUniquePredict,
    detailImage: detailUniquePredict,
    summary:
    ...
    problem:
      'Most football tip sites either overcomplicate things or make promises they can’t keep. UniquePredict had to publish a prediction for every fixture across 30 leagues, every day, with a method you can actually follow, and keep an honest record of how it does.',
    approach: [
      'One model per fixture: a Poisson distribution with a Dixon-Coles adjustment for tight, low-scoring games, built from each team’s recent scoring and defensive form and weighted so recent matches count for more.',
      'Every market (result, goals lines, both teams to score, correct score) is read straight off that one score grid, so the numbers never contradict each other.',
      'A confidence figure on every pick, capped at 92% because nothing in football is certain.',
      'A public results archive where losing tips are never removed or edited.',
    ],
    outcome: [
      'A daily pipeline that scores upcoming fixtures across 30 competitions and publishes early enough to check before kickoff.',
      'Fast and readable on a phone, with a Premium tier for the model’s highest-rated picks.',
    ],
  },
  {
    slug: 'voiceiq',
    ...
    role: 'Platform architecture and build',
    url: 'https://aivoiceagenttesting.com',
    image: caseVoiceIQ,
    detailImage: detailVoiceIQ,
    summary:
    ...
    problem:
      'Teams ship AI voice agents and only find the failures when a customer does. VoiceIQ had to test an agent the way a customer would, with a real phone call, and prove whether it actually did what it said it would.',
    approach: [
      'Quinn AI generates test scenarios from the agent’s profile and a scrape of its own website, so every test is grounded in the real business.',
      'A tester agent places the calls; GPT-4o scores the transcripts against what the scenario expected, with dual speech-to-text for accuracy.',
      'Action Verification: when an agent says it will send an email or an SMS, a dedicated Gmail and Twilio inbox confirm it arrived, or the test fails.',
      'Adversarial “Chaos” testing across eight research-backed vulnerability classes, plus compliance audits and a SOC 2 evidence pack.',
    ],
    outcome: [
      'Scenario generation, live call orchestration, scored transcripts and a certification program, all in one platform.',
      'Integrates with the major voice platforms and auto-provisions a dedicated phone number per agent.',
    ],
  },
  {
    slug: 'workflowauth',
    ...
    role: 'Productized service, build and delivery',
    url: 'https://app.workflowauth.com',
    image: caseWorkflowAuth,
    detailImage: detailWorkflowAuth,
    summary:
    ...
    problem:
      'Local service businesses lose jobs every time a call goes to voicemail at 6:01pm. WorkflowAuth had to answer every one of those calls, qualify the lead and get it on the calendar, without the owner lifting a finger.',
    approach: [
      'An AI receptionist that answers inbound within two rings in a voice the business chooses, qualifies the caller and books the job.',
      'Speed-to-lead: new leads get a callback or text within seconds, before a competitor picks up.',
      'Outbound too: cold leads re-engaged, quiet quotes revived, appointment reminders placed automatically.',
      'Done-for-you setup: their script, their hours, their CRM, wired to inbound answering and outbound callbacks and tested end to end.',
    ],
    outcome: [
      'A productized service with a marketing site, a revenue calculator and CRM-integrated call handling.',
      'Live for a client in about three days.',
    ],
  },
]
          ...
          <section className="build-quiz section">
            <div className="container build-quiz-box reveal-up">
              ...
              <div className="quiz-summary">
                <p>
                  {selectedBuild}, to {selectedGoal.toLowerCase()}.
                </p>
